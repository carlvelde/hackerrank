...
public class IsBinarySearchTree {
    ...
    public static boolean isBST(TreeNode node) {
        return checckBst2(node, null, null);
    }

    private static boolean checkBST(TreeNode node, Integer max, Integer min) {

        if (max != null && node.value < max) // !(
            return false;
        if (min != null && min < node.value) // !(node.value > lowlimit)
            return false;

        boolean isLeftBst = true;
        boolean isRightBst = true;
        if (node.left != null) {
            isLeftBst = checkBST(node.left, max, node.value);
        }
        if (isLeftBst && node.right != null) {
            isRightBst = checkBST(node.right, node.value, min);
        }
    ...
    private static boolean checckBst2(TreeNode node, Integer lowLimit, Integer highLimit) {
        ...
        if (node.left != null) {
            leftIsBst = checckBst2(node.left, lowLimit, node.value);
        }
        if (leftIsBst && node.right != null) {
            rightIsBst = checckBst2(node.right, node.value, highLimit);
        }
